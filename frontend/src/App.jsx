...
function ParameterPanel({ form, loading, exporting, canExport, error, onChange, onCalculate, onExport }) {
...
export default function App() {
  ...
  const exportPdf = async () => {
    ...
    try {
      const response = await fetch(`${apiBaseUrl}/api/export/pdf`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
      if (!response.ok) {
        const result = await response.json().catch(() => ({}))
        throw new Error(result.detail || 'Não foi possível gerar o PDF.')
      }
      const blob = await response.blob()
      const url = URL.createObjectURL(blob)
      const link = document.createElement('a')
      link.href = url
      link.download = 'lgr-resolucao.pdf'
      document.body.appendChild(link)
      link.click()
      link.remove()
      URL.revokeObjectURL(url)
    } catch (requestError) {
      setError(requestError.message || 'Não foi possível gerar o PDF.')
  ...
  return <div className="app">
    ...
    <main><section className="intro"><div><p className="eyebrow">DCA-3701 · UFRN</p><h2>Lugar Geométrico das Raízes</h2><p>Explore a resolução completa de forma visual e interativa.</p><p>Aluno : Carlos Eduardo Medeiros da Silva</p></div><div className="status"><span className="dot" /> API conectada</div></section><div className="layout"><ParameterPanel form={form} loading={loading} exporting={exporting} canExport={Boolean(data)} error={error} onChange={update} onCalculate={calculate} onExport={exportPdf} /><section className="results">{data ? <AnalysisSummary data={data} /> : <div className="empty"><Activity size={42} /><h3>Pronto para analisar</h3><p>Informe os parâmetros e clique em calcular para visualizar a resolução.</p></div>}</section></div></main>
